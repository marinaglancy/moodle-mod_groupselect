...
$canviewmembers = has_capability('mod/groupselect:viewmembers', $context);
...
if ($canassign and count($groups) > 0 )  {
    ...
    $button->add_confirm_action(get_string('assigngroup_confirm', 'mod_groupselect'));
...
if (empty ( $groups )) {
	...
	foreach ( $groups as $group ) {
		...
		// Group members
		if ($canviewmembers && $canseemembers) {
                        $membernames = array ();
	...
	if ($actionpresent) {
        ...
        if (!$canviewmembers) {
            array_splice($table->head, 2, 2);
            array_splice($table->size, 2, 2);
            array_splice($table->align, 2, 2);
        }
	$table->data = $data;
